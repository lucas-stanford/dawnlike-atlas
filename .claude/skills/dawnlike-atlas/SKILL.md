...
- **4,487 sprites** packed into a single `2048×2272` PNG (`64×71` grid). 4,157 are original DawnLike art; the remaining 330 are generated — 235 `* shore` coastline tiles, 64 `* watered field` soil tiles, and 31 ship pieces (`ship deck *` planks, a 16-tile `ship rail *` gunwale family, `ship bow *` prows and a `ship mast`).
...
- `src/SpriteBrowserExample.jsx` — search all 4,487 sprites by name and tag; also the reference usage of `src/utils/atlasApi.js`.
