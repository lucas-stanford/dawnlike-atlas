...
- **One-cell sailing boats** — `sloop n|e|s|w` and `black sloop n|e|s|w`, a
  ...
  Drawing a sail from directly overhead is where this set knowingly cheats. Seen
  truly from above a sail is edge-on and the boat is a hull with a scratch on
  it; a round sail laid over the middle ate the hull and read as a dumpling, and
  a slim sail set entirely outboard read as a fin. The canvas is bellied out to
  one side with its luff bent to the mast, which is how top-down games have
  drawn one since the 8-bit era.
