...
export const AUTOTILE_MANIFESTS = {
  ...
  wall: {
    ...
    // NOTE: fallback entries are PATTERN KEYS, not suffixes. The
    // isolated-tile pattern key is '' (which maps to the `center`
    // suffix) — writing 'center' here would silently never match.
    fallbacks: {
      'nsew': ['ew', 'ns', ''],
      'nse':  ['ns', ''],
      'nsw':  ['ns', ''],
 ...
 *
 * Land ↔ water coastline resolver for the `* shore` families
 * (`sand shore`, `grass shore`, `snow shore`, `mud shore`, `ash shore`).
 *
 * These sprites are NOT part of the original DawnLike pack — the pack
 * ships no coastline art, so they are generated by
 * `scripts/generate-shore.mjs`. A shore tile carries the whole land →
 * water transition inside one cell and is TRANSPARENT where the water
 ...
 * you would give the floor resolver:
 *
