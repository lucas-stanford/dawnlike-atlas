...
export default function AutotileLabExample({
  ...
  const spriteBox = (name, boxScale, extraClass = '') => {
    ...
    if (!sprite) {
      return <div className={`lab-sprite lab-sprite-missing ${extraClass}`} style={{ width: tile * boxScale, height: tile * boxScale }} />;
    }
            ...
            {ALL_PATTERNS.map((nb) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setNeighbors((prev) => ({ ...prev, ...nb }))}
                  title={res.name}
                >
